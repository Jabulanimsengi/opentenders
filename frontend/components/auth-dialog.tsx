...
export function AuthDialog({ triggerClassName }: AuthDialogProps = {}) {
  ...
  const [signInError, signInDispatch, isSigningIn] = useActionState(
    authenticate,
    undefined,
  );
  const [signUpError, signUpDispatch, isSigningUp] = useActionState(
    register,
    undefined,
  );
  ...
  const isSuccessMessage = errorMessage?.startsWith("Account created.");
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <button
          className={cn(
            "cursor-pointer rounded-lg px-4 py-2 text-sm font-medium text-gray-600 transition-all hover:bg-emerald-50 hover:text-emerald-600",
            triggerClassName,
          )}
        >
          Sign In
      ...
      <DialogContent className="sm:max-w-[380px] p-0 border-0 bg-transparent shadow-none">
        ...
        <Card className="w-full bg-white shadow-2xl border-0">
          ...
          <CardContent className="pt-0 pb-4 px-5">
            ...
            {errorMessage && (
              <div
                className={`mt-2 text-xs text-center py-1.5 px-2 rounded ${isSuccessMessage ? "text-emerald-700 bg-emerald-50" : "text-red-600 bg-red-50"}`}
              >
                {errorMessage}
              </div>
            )}
